...
Type strComputer
	...
	Declare Constructor(nNetworkAddress As Longint) 'The computer should have support for large numbers...
...
Constructor strComputer(nNetworkAddress As Integer)
	For K As Integer = 0 To nUpperBound
		aProgramIntCode(K) = aOriginalProgram(K)
	Next K
	AddToInputBuffer(nNetworkAddress) 'when each computer boots up, it will request its network address via a single input instruction.
End Constructor
