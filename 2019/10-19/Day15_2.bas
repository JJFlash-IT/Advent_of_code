...
Redim aOxygenMap(-22 To 22, -22 To 22) As strMap 'This is to be used for part 2...

aOxygenMap(0, 0).Tile = 1 'The starting position is surely walkable...
...
Dim tOxygenStartingCoords As strCoordinates

Dim As Longint nDroidStatus, nThisDirection
Do
	For nThisDirection = aMap(tDroid.Row, tDroid.Col).LastDir + 1 To 4
		If nThisDirection = aMap(tDroid.Row, tDroid.Col).BacktrackDir Then Continue For 'It's useless exploring the location the droid has just come from!
		...
		aMap(tDroid.Row, tDroid.Col).LastDir = nThisDirection
		tDroid += aDirections(nThisDirection)
		aOxygenMap(tDroid.Row, tDroid.Col).Tile = nDroidStatus
		If nDroidStatus = 2 Then              '2: The repair droid has moved, its new position is the location of the oxygen system
			tOxygenStartingCoords = tDroid
			Print "Found oxygen tank at coordinates: " & tOxygenStartingCoords.Row & "," & tOxygenStartingCoords.Col
		EndIf
		aMap(tDroid.Row, tDroid.Col).BacktrackDir = aDirBacktrack(nThisDirection)
		Continue Do 'Next recursion!
	Next nThisDirection
	'Backtrack!
	nThisDirection = aMap(tDroid.Row, tDroid.Col).BacktrackDir
	If nThisDirection = 0 Then Exit Do 'Went back to the starting location, map should be complete!
	RunProgram(nThisDirection, nDroidStatus)
...
tDroid = tOxygenStartingCoords
Dim nOxygenMinutes As Integer
Do
	For nThisDirection = aOxygenMap(tDroid.Row, tDroid.Col).LastDir + 1 To 4
		If nThisDirection = aOxygenMap(tDroid.Row, tDroid.Col).BacktrackDir Then Continue For 'It's useless exploring the location the oxygen has just come from!
		If aOxygenMap(tDroid.Row + aDirections(nThisDirection).Row, tDroid.Col + aDirections(nThisDirection).Col).Tile = 0 Then Continue For 'The oxygen "hit" a wall
		nSteps += 1 'The oxygen "moves"
		If nSteps > nOxygenMinutes Then nOxygenMinutes = nSteps
		aOxygenMap(tDroid.Row, tDroid.Col).LastDir = nThisDirection
		tDroid += aDirections(nThisDirection)
		aOxygenMap(tDroid.Row, tDroid.Col).BacktrackDir = aDirBacktrack(nThisDirection)
		Continue Do 'Next recursion!
	Next nThisDirection
	'Backtrack!
	nThisDirection = aOxygenMap(tDroid.Row, tDroid.Col).BacktrackDir
	If nThisDirection = 0 Then Exit Do 'Went back to the starting location, map should be FILLED!
	nSteps -= 1 'Since the droid is backtracking, so is the number of steps...
	tDroid += aDirections(nThisDirection)
Loop
Print : Print "Minutes to fill the map with oxygen: " & nOxygenMinutes