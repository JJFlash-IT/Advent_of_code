...
Open Exepath + "\input16_part2test1.txt" For Input As #1
	Scope
...
For nPhaseCount = 1 To 100
	For nOutElementIndex = 1 To Ubound(aInputSignal)
		...
		For nInElementIndex = 1 To Ubound(aInputSignal)
			...
			End If
			Print "" & aInputSignal(nInElementIndex, nSourceSignal) & "*" & aPattern(nPatternIndex).Value & Iif(aPattern(nPatternIndex).Value < 0, " ", "  ") ;
			If nInElementIndex < Ubound(aInputSignal) Then Print "+ ";
